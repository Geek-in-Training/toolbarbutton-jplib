...
function init (pref) {
    ...
    // work around bug #627432
    
    if (self.loadReason === "install") {
        removePrefs(); /* because of bug #627432 the prefrences might not be removed at uninstall. */
    } 
    else if ( prefs.isSet(PREF + "list") ) {
...
function eachToolbar(callback) {
    for (i = 0, w = newWinUtils.windows(), l = w.length; i < l; i++) {
        let $ = function (id) {
            return w[i].document.getElementById(id);
        }
        
        for (i = 0; i < prefList.length; i++) {
            let toolbarID = prefList[i].replace(/\.currentset$/, "");
            if (tb = $(toolbarID)) {
                callback(tb);
            }
        }
...
function restoreButtons (tb) {
    ...
    if (currentSet) {
        ...
        updatePrefs(tb.id + ".currentset", currentSet);
...
function onShutdown (reason) {
    if (reason == "uninstall") { /* does not happen, due to bug #627432 */
        removePrefs();
    } else {
